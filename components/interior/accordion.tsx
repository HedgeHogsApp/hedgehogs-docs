...
/**
 * Interior.dev accordion — token-mapped to Hedgehog's design system.
 *
 * Source: ddoemonn/interior/components/interior/accordion.tsx
 * Behavior is identical to upstream: spring-measured height via ResizeObserver
 * (`useAutoHeight`), roving focus with Home/End, `inert` on closed panels,
 * configurable heading level, and a `meta` slot. The hardcoded `stone-*`
 * palette / `#4568FF` focus ring are mapped to Hedgehog semantic tokens
 * (fg-*, surface-*, line, brand).
 *
 * Exports the headless `useAccordion` hook AND the styled `Accordion` example
 * (the replaceable styled export). Consumers that need a different visual wrap
 * the hook directly.
 */
...
import { CHEVRON, DISCLOSE, EASE, EXIT_EASE } from "@/lib/motion";
...
export function Accordion({
  ...
  return (
    <div
      className={`divide-y divide-line overflow-hidden rounded-[11px] border border-line bg-surface-card shadow-[0_1px_2px_rgba(28,25,23,0.06),0_4px_10px_-8px_rgba(28,25,23,0.45)] dark:divide-line dark:border-line dark:bg-surface-panel dark:shadow-[0_1px_6px_rgba(0,0,0,0.45)] ${className}`}
    >
      {items.map((item) => (
        <AccordionRow
          key={item.id}
  ...
  return (
    <div>
      <div role="heading" aria-level={headingLevel}>
        <button
          {...header}
          className="flex w-full items-center gap-3 px-3.5 py-3 text-left outline-hidden transition-colors duration-150 hover:bg-surface-overlay focus-visible:bg-brand/[0.06] focus-visible:shadow-[inset_0_0_0_1px_rgb(var(--brand))] dark:hover:bg-white/10 dark:focus-visible:bg-brand/[0.1] dark:focus-visible:shadow-[inset_0_0_0_1px_rgb(var(--brand))]"
        >
          <span className="min-w-0 flex-1 truncate text-[13px] font-medium text-fg-primary transition-colors duration-150">
            {item.title}
          </span>
      ...
      >
        <div
          {...panel}
          ref={ref}
          className="border-t border-line bg-surface-panel shadow-[inset_0_1px_2px_rgba(28,25,23,0.05)] dark:border-line dark:bg-white/[0.05] dark:shadow-[inset_0_1px_2px_rgba(0,0,0,0.3)]"
          style={{
        ...
        >
          <motion.div
            ...
            className="px-3.5 pb-3.5 pt-3 text-[12.5px] leading-relaxed text-fg-muted"
          >
            {item.content}
          </motion.div>
        </div>
